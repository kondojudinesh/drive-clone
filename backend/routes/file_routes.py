...
def file_signed_url(file_id):
    ...
    if not file_row:
        # allow if shared via permissions (by email)
    ...
    return jsonify({"signed_url": signed, "file": {"id": file_row["id"], "filename": file_row["filename"]}}), 200
...
def restore_from_trash(file_id):
    ...
    supabase.table("files").update({"is_deleted": False, "trashed_at": None}).eq("id", file_id).execute()
...
def share_file(file_id):
    ...
    share_link = f"{request.host_url.rstrip('/')}/public/{token}"
...
def access_shared_file(share_token):
    ...
    return jsonify({
        "file": {"id": file_row["id"], "filename": file_row["filename"], "type": file_row.get("type")},
        "signed_url": signed,
...
def update_permissions(file_id):
    ...
    supabase.table("files").update({"permissions": {"viewer": viewers, "editor": editors}}).eq("id", file_id).execute()
